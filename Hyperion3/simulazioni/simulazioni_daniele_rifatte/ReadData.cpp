...
int main(int argc, char* argv[]) {
  ...
  if(argc!=2){
    std::cout << "Manca nome della cartella" << std::endl;
    std::cout << "Example: ./ReadData data_avanti_65V" << std::endl;
    exit(1);
  }
  std::string path = Form("/Users/massimo/Documents/phd/ANDROMeDa/Hyperion3/simulazioni/simulazioni_daniele_rifatte/data/%s",folder);
  ...
  for (int i = 0; i < filenames.size(); i++) {
    ...
    std::string name_str = AndCommon::splitString(filenames[i], "/"   )[10];
